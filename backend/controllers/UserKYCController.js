...
export const updateUserKYCStatus = async (req, res) => {
    try {
        ...
        // Send email notification when status changes to verified or rejected
        if (status === "verified" || status === "rejected") {
            try {
                // Create appropriate URLs
                const baseUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
                const loginUrl = `${baseUrl}/login`;
                const kycSubmitUrl = `${baseUrl}/kyc-submit`;

                // Set email subject based on status
                const emailSubject = status === "verified"
                    ? "Your TripShare KYC Verification is Approved!"
                    : "Action Required: TripShare KYC Verification";
                ...
                await sendEmail({
                    ...
                    context: {
                        name: user.fullName,
                        status,
                        rejectionReason: userKYC.kycRejectionReason,
                        loginUrl: status === "verified" ? loginUrl : kycSubmitUrl
                    }
                });
